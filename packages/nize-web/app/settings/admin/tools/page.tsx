...
function AdminServerList({ servers, groupBy, onDelete, onToggleEnabled }: { servers: AdminServerView[]; groupBy: "visibility" | "transport"; onDelete: (serverId: string) => void; onToggleEnabled: (serverId: string, enabled: boolean) => void }) {
...
function CreateServerForm({ onSubmit, onTest, onCancel }: { onSubmit: (config: { name: string; domain: string; description: string; visibility: string; transport: string; url?: string; command?: string; args?: string[]; authType?: string; apiKey?: string }) => Promise<void>; onTest: (config: { url?: string; transport: string; authType?: string; apiKey?: string }) => Promise<{ success: boolean; toolCount?: number; error?: string }>; onCancel: () => void }) {
  ...
  const handleTest = async () => {
    ...
    try {
      const result = await onTest({
        ...
        authType: transport === "http" ? authType : undefined,
        apiKey: transport === "http" && authType === "api-key" ? apiKey : undefined,
      });
...
export default function AdminToolsPage() {
  ...
  const handleCreate = async (config: { name: string; domain: string; description: string; visibility: string; transport: string; url?: string; command?: string; args?: string[]; authType?: string; apiKey?: string }) => {
    const res = await authFetch("/mcp/admin/servers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <div>
      <div className="mb-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={() => setShowCreateForm(!showCreateForm)} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">
